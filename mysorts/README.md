# [Golang] 排序
学习各种排序的实验代码, 主要参考《算法导论 第3版》, 包括插入排序(Insertion Sort)、归并排序(Merge Sort)、堆排序(Heap Sort)、快速排序(Quick Sort)等比较排序算法以及计数排序(Counting Sort)、基数排序(Radix Sort)、桶排序(Bucket Sort)等非比较排序算法.     
...
## 实验代码
- mysorts    
实现各种排序算法的实验代码的包, 基于`sort.Interface`来实现. (可通过命令`go test -v -bench=. -benchmem`来进行功能测试和Benchmark)     
